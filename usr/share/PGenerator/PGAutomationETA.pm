...
sub timing_profile {
 ...
 my @keys=$stage eq 'greyscale-done'
  ...
  : ();
 $p{calibration}={map {$_=>defined($cal->{$_})?"$cal->{$_}":''} @keys};
...
sub update {
 ...
 my $identity=JSON::PP->new->canonical->encode([$index,$stage,$run->{stage_started_at},$run->{resumed_at},$worker->{status},$worker->{current_step},$worker->{total_steps},$clock,
