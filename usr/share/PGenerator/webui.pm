  ...
  my $lim=$greyscale_patch_limited;
   @steps=map {
    ...
    my $c=0;
     if($dv_series) {
       if($dv_map_mode eq "1") {
       # DV absolute greyscale keeps the tunnel transport full-range, but some
       # sinks still clamp interior greyscale steps against the legal-range
       # floor. Keep the interior steps on nominal legal codes while
       # preserving explicit full-range black/white endpoints.
       my $level=int($stimulus_for_slot{$v}/100*219+.5);
       if($lim) {
        $c=$level+16;
       } elsif($stimulus_for_slot{$v} <= 0) {
        $c=0;
       } elsif($stimulus_for_slot{$v} >= 100) {
        $c=255;
       } else {
        $c=$level+16;
       }
...
sub webui_pattern_effective_bits (@) {
 ...
 # Dolby Vision patches stay on the 8-bit RGB tunnel path even when the HDMI
 # link is driven at 12 bpc.
 return 10 if(($signal_mode eq "hdr10" || $signal_mode eq "hlg") && $link_bits >= 10);
...
color:var(--text2);transition:color .18s,filter .18s,transform .08s,background .18s}
.icon-btn:hover{background:rgba(255,255,255,.05)}
.icon-btn:active{transform:scale(.94)}
.icon-btn svg{display:block}
.icon-btn[disabled]{cursor:not-allowed;opacity:.55}
/* Power button — green glow when the Pi heartbeat is alive, muted grey
   when we know it's going down or the heartbeat is stale. */
.power-btn.online{color:#4caf50;filter:drop-shadow(0 0 5px rgba(76,175,80,.65))}
.power-btn.online:hover{color:#5dd362;filter:drop-shadow(0 0 7px rgba(76,175,80,.85))}
.power-btn.offline{color:#555a64;filter:none}
.power-btn svg{transform:translateY(1px)}
.reboot-btn{color:#b6b8c2}
...
#meterSettingsGrid .field label.meter-field-label{white-space:normal}
#meterSettingsGrid .field > select,
...
.meter-matrix-grid input{text-align:center}
...
<div class="header">
 ...
 <div class="hdr-right">
  ...
  <button class="icon-btn power-btn offline" id="pwrBtn" aria-label="Power off" title="Power off" onclick="shutdownDevice()">
   ...
   <button class="icon-btn reboot-btn" id="rebootBtn" aria-label="Reboot" title="Reboot" onclick="confirmReboot()">
    <svg viewBox="0 0 24 24" width="22" height="22" aria-hidden="true"><path d="M19 12a7 7 0 1 1-2.05-4.95" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"/><path d="M19 4.65V8.85H14.8" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/></svg>
   </button>
  </div>
  ...
  <div class="grid">
   <div class="field">
    ...
    <select id="dv_map_mode">
     ...
     <div class="field">
      <label>MaxCLL</label>
      <input type="number" id="dv_max_cll" min="0" max="10000" step="1">
     </div>
     <div class="field">
      <label>MaxFALL</label>
   ...
   <div style="width:min(920px,100%);max-height:92vh;overflow:auto;background:#111723;border:1px solid #2a3140;border-radius:12px;padding:16px;box-sizing:border-box;box-shadow:0 18px 60px rgba(0,0,0,.45)">
    ...
    <div style="display:grid;grid-template-columns:minmax(260px,320px) minmax(0,1fr);gap:16px;align-items:start">
     <div style="background:#161d2a;border:1px solid #2a3140;border-radius:10px;padding:12px">
      ...
      <div id="ccssUploadStatus" style="font-size:.72rem;color:var(--text2);margin-top:6px;min-height:1.1em"></div>
      <div style="margin-top:12px;padding-top:10px;border-top:1px solid #2b2f42">
       <div style="font-size:.74rem;color:var(--text2);margin-bottom:6px">Custom profiles</div>
    ...
    <div style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:10px;margin-bottom:10px">
     <div>
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;flex-wrap:wrap">
       ...
       <label style="font-size:.7rem;color:var(--text2);cursor:pointer;user-select:none;margin-left:auto" title="Switch EOTF Y-axis to logarithmic scaling for better low-light visibility.">
...
function isSettingsFieldActive(){
 ...
 return ['mode_idx','signal_mode','max_bpc','color_format','colorimetry','rgb_quant_range',
  'eotf','primaries','dv_map_mode','max_luma','min_luma','max_cll','max_fall',
...
async function checkPing(){
 ...
 try{
  ...
  if(wasOffline){
   ...
   document.getElementById('statusWrap').title='Connection restored, refreshing...';
...
function meterPopulateRoleSelects(meters,detectedPort){
 ...
 meterMeasurementPort=meterNormalizePortValue((document.getElementById('meterMeasurementPort')||{}).value)||savedMeasurementPort;
...
function meterRecoverSeries(s){
 ...
 if(s.white_reading&&s.white_reading.luminance!=null){
...
function meterFindSeriesWhiteReading(readings){
 ...
 return list.find(rd=>{
  ...
  const g=(rd.g_code!=null)?rd.g_code:rd.g;
  const b=(rd.b_code!=null)?rd.b_code:rd.b;
  return r!=null && g!=null && b!=null && (((rd.ire||0)===100) || name==='white') && Number(r)===Number(g) && Number(g)===Number(b);
 });
}
...
async function meterDisplayPatch(step){
 ...
 await fetchJSON('/api/pattern',{method:'POST',headers:{'Content-Type':'application/json'},
  body:JSON.stringify(meterMeasurementSignalContext({name:'patch',r:step.r,g:step.g,b:step.b,size:psize,input_max:255,signal_range:signalRange||undefined})),_timeoutMs:5000});
}
...
function meterRedrawEotfChart(){
 ...
 const readingMap={};
 meterReadings.forEach(r=>{if(r.luminance!=null&&r.r_code===r.g_code&&r.g_code===r.b_code) readingMap[r.ire]=r;});
...
function drawEOTFPreset(gsSteps){
 ...
 const tgtPts=(gsSteps||[]).map((s,idx)=>[meterGreyChartX(s,gsSteps,idx),meterGreyTargetEotfValue(s.ire,Lw,0,s.r_code!=null?s.r_code:s.r)/yTop]);
 drawDashedLine(ctx,chart,tgtPts,'#666');
 const pts=gsSteps.map((s,idx)=>{
  const v=meterGreyChartX(s,gsSteps,idx);
...
function drawGammaValuePreset(gsSteps){
 ...
 steps.forEach((s,idx)=>{
  ...
  const anchor=meterGammaPlotAnchorStep(gsSteps,idx);
  const anchorIdx=idx>0?idx-1:idx;
  if(g!=null&&isFinite(g)&&anchor) tgtPts.push([meterGreyChartX(anchor,gsSteps,anchorIdx),Math.max(0,Math.min(1,(g-yMin)/(yMax-yMin)))]);
 });
...
function drawGammaValueChart(gs,allSteps,readingMap){
 ...
 sorted.forEach((rd,idx)=>{
  ...
  const g=(meterChartIsDv() && meterDvMapModeValue()==='2' && ((document.getElementById('meterTargetGamma')||{}).value)==='st2084' && (rd.ire||0)>=100)
   ? meterDvRelativeCalmanWhiteGamma(y,Yw)
   : effectiveGamma(y,Yw,rd.ire,prevY,prevIre);
...
function drawEOTFChart(gs,allSteps,readingMap){
 ...
 const targetPeak=meterGreyTargetPeak(refWhite);
 const Lb=meterChartBlackLevel(sorted);
 if(!(Math.max(refWhite,targetPeak)>0)) return;
 ...
 targetSteps.forEach(s=>allRatios.push(meterGreyTargetEotfValue(s.ire,targetPeak,Lb,s.r_code!=null?s.r_code:s.r)));
 const yTop=Math.max(0.55,Math.ceil(Math.max(...allRatios)*1.12*20)/20);
 const chart=drawChartGrid(ctx,{
  pad:{t:34,r:15,b:30,l:55},
  xSteps:10,ySteps:5,
  xLabel:(i)=>(i*10)+'',
  yLabel:(i,n)=>meterEotfUnscaleValue(i/n,yTop).toFixed(2)
 });
 const tgtPts=targetSteps.map((s,idx)=>[
  meterGreyChartX(s,targetSteps,idx),
  meterEotfScaleValue(meterGreyTargetEotfValue(s.ire,targetPeak,Lb,s.r_code!=null?s.r_code:s.r),yTop)
 ]);
 drawDashedLine(ctx,chart,tgtPts,'#666');
 if(allSteps){
  const emptyPts=[];
  allSteps.forEach((s,idx)=>{
   ...
   const v=meterGreyChartX(s,allSteps,idx);
  emptyPts.push([v,meterEotfScaleValue(meterGreyTargetEotfValue(s.ire,targetPeak,Lb,s.r_code!=null?s.r_code:s.r),yTop)]);
  });
  drawDots(ctx,chart,emptyPts,'#33333380',3);
 }
 const measureSteps=allSteps||valid;
 const mSegments=meterSeriesMeasuredSegments(measureSteps,readingMap,(reading,step,idx)=>[
  meterGreyChartX(step,measureSteps,idx),
  meterEotfScaleValue(meterGreyMeasuredEotfValue(reading.luminance||0,refWhite),yTop)
 ]);
 if(!allSteps && mSegments.length===1) mSegments[0]=meterAnchorOriginPoint(mSegments[0]);
...
function drawGammaChart(gs,allSteps,readingMap){
 ...
 const Lb=meterChartBlackLevel(sorted);
 const rawTargetPeak=meterGreyTargetPeak(refWhite||measuredMax);
 const targetPeak=(meterChartIsDv() && meterDvMapModeValue()==='1' && refWhite>0) ? refWhite : rawTargetPeak;
 if(!(Math.max(measuredMax,targetPeak)>0)) return;
 const yTop=Math.ceil(Math.max(measuredMax,targetPeak)*1.1/10)*10||Math.max(measuredMax,targetPeak);
 const chart=drawChartGrid(ctx,{
  pad:{t:20,r:15,b:30,l:55},
  xSteps:10,ySteps:5,
  xLabel:(i)=>(i*10)+'',
 ...
 const tgtPts=targetSteps.map((s,idx)=>[
  meterGreyChartX(s,targetSteps,idx),
  meterLuminanceScaleValue(meterGreyTargetChartValue(s.ire,targetPeak,Lb,s.r_code!=null?s.r_code:s.r),yTop)
 ]);
 drawDashedLine(ctx,chart,tgtPts,'#666');
 if(allSteps){
  allSteps.forEach((s,idx)=>{
   if(readingMap[s.ire]) return;
   const v=meterGreyChartX(s,allSteps,idx);
   drawDots(ctx,chart,[[v,meterLuminanceScaleValue(meterGreyTargetChartValue(s.ire,targetPeak,Lb,s.r_code!=null?s.r_code:s.r),yTop)]],'#33333380',3);
  });
 }
 const validG=sorted.filter(r=>r.luminance!=null && r.luminance>=0);
 const measureSteps=allSteps||validG;
 const mSegments=meterSeriesMeasuredSegments(measureSteps,readingMap,(reading,step,idx)=>[
  meterGreyChartX(step,measureSteps,idx),
 ...
 ctx.fillText('100% target: '+targetPeak.toFixed(1)+' cd/m\u00B2',chart.pad.l,chart.pad.t+14);
 ctx.textAlign='right';
...
function drawDeltaEChart(gs,allSteps,readingMap){
 ...
 if(whiteR&&whiteR.x>0&&whiteR.y>0){
  const wp=meterTargetWhitePoint();
  const dxy=Math.sqrt(Math.pow(whiteR.x-wp.x,2)+Math.pow(whiteR.y-wp.y,2));
...
function drawColorReadingsTable(readings){
 ...
 readings.forEach(rd=>{
  ...
  html+='<td style="padding:5px 6px;text-align:right;font-weight:600;color:'+deCol+'">'+(hasDe?de.toFixed(2):'--')+'</td>';
  html+='</tr>';
 });
 if(deCount>0){
...
function drawColorSeriesAverages(readings,colorRefMode){
 ...
 const rowFor=(label,g,bold)=>{
  ...
  const c=!Number.isFinite(aDE)?'#888':aDE<1?'#4caf50':aDE<3?'#ff9800':'#f44';
  const cMax=!Number.isFinite(mDE)?'#888':mDE<1?'#4caf50':mDE<3?'#ff9800':'#f44';
  ...
  r+='<td style="padding:5px 6px;text-align:right;color:'+c+'">'+(Number.isFinite(aDE)?aDE.toFixed(2):'--')+'</td>';
  r+='<td style="padding:5px 6px;text-align:right;color:'+cMax+'">'+(Number.isFinite(mDE)?mDE.toFixed(2):'--')+'</td>';
  r+='</tr>';
  return r;
 };
 let html='';
...
function colorChartRegisterInteraction(readings){
 _colorChartHitZones=[];
 if(!readings||readings.length===0) return;
 const plotReadings=readings.filter(rd=>!meterIsWhiteReferenceReading(rd));
 if(plotReadings.length===0) return;
 ...
 if(deCanvas){
  ...
  const cW=rect.width;
  const valid=plotReadings.filter(rd=>meterReadingHasLuminance(rd));
...
function meterIsSelectedColorReading(rd){
