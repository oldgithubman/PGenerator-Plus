...
sub preserve_unconfirmed_lg_calibration_session {
 my ($state,$response,$write_name)=@_;
...
eval {
 ...
 if($upload_requested && !$config->{"fixture_mode"}) {
  ...
  if(ref($probe) eq "HASH" && $probe->{"status"} eq "ok" && $probe->{"upload_supported"}) {
   ...
   $state->{"upload"}=$upload;
   preserve_unconfirmed_lg_calibration_session($state,$upload,"3D LUT write");
   ...
   $state->{"upload_error_code"}=(ref($upload) eq "HASH") ? ($upload->{"error_code"}||"") : "";
   ...
   $state->{"upload_json_error"}=($upload_message=~/Invalid Web UI API response|LG helper execution failed/i) ? json_true() : json_false();
    ...
    if($hold_for_hdr_tone_map && ($state->{"upload_status"}||"") eq "ok" && $state->{"upload_verified"}) {
     ...
     if($tone_peak > 0) {
      ...
      preserve_unconfirmed_lg_calibration_session($state,$tone_resp,"HDR tone-map write");
      ...
      $state->{"tone_map_upload_error_code"}=(ref($tone_resp) eq "HASH") ? ($tone_resp->{error_code}//"") : "";
     ...
     log_line("3D LUT stage: full_workflow tone map upload skipped -- no peak luminance in config");
      $state->{"tone_map_upload_status"}="error";
      $state->{"tone_map_upload_error_code"}="lg-tone-map-peak-missing";
      $state->{"tone_map_upload_message"}="HDR tone-map finalisation cannot continue because the 3D worker did not receive a measured peak luminance";
      $state->{"tone_map_uploaded"}=json_false();
      $state->{"calibration_mode"}=json_true();
      $state->{"calibration_end_retry_forbidden"}=json_true();
      $state->{"calibration_recovery_required"}=json_true();
      $state->{"calibration_recovery_message"}="HDR tone-map finalisation is missing the measured peak luminance. No fallback CAL_END will be sent from another connection. Restart the TV before another calibration.";
      write_state($state);
     }
    }
   } else {
 ...
 if(lc(($config->{"signal_mode"}||"")) eq "hdr10"
  ...
  eval {
   ...
   preserve_unconfirmed_lg_calibration_session($state,$shadow_lut,"HDR shadow 3D LUT write");
   ...
   preserve_unconfirmed_lg_calibration_session($state,$shadow_tone,"HDR shadow tone-map write");
 ...
 # ---- Final shadow smoothing: the LAST thing that touches the 1D DPG ----
 # Everything upstream that rewrites the curve has run by now: the 3D LUT
 # upload, the HDR tone-map upload (which carries its own dpg_data payload), the
 # post-cal shadow fix (which uploads a corrected curve and refreshes
 # full_workflow_dpg_data), and post_check. Smoothing earlier -- in the
 ...
 if(!cancelled()) {
  ...
  if(ref($final_dpg) eq "ARRAY") {
   ...
   if($changed) {
    ...
    write_state($state);
    my $layout=lc($config->{"ddc_layout"}||"");
    my $smooth_resp=api_json("POST","/api/lg/1d-dpg/upload",{
     picture_mode => $config->{"picture_mode"}||"",
     ddc_layout => ($layout ne "" ? $layout : "hdr20"),
    ...
    my $sm_msg=(ref($smooth_resp) eq "HASH")
     ? ((ref($smooth_resp->{message}) eq "ARRAY") ? join(" ",@{$smooth_resp->{message}}) : ($smooth_resp->{message}||""))
     : "no response";
    $state->{"dpg_shadow_smoothed"}=$sm_ok ? JSON::PP::true : JSON::PP::false;
    $state->{"dpg_shadow_smoothed_entries"}=$changed+0;
    $state->{"dpg_shadow_smoothed_message"}=$sm_msg;
    if($sm_ok) {
 ...
 # A generic dropped upload may strand CAL_START, but an accepted write with
 # unconfirmed CAL_END must never receive a fallback close from this worker's
 # separate API connection.
 if($upload_requested && !$config->{"fixture_mode"} && !lg_calibration_end_retry_forbidden($state)) {
 ...
 } elsif($upload_requested && !$config->{"fixture_mode"}) {
  $state->{"calibration_mode_cleanup_skipped"}=json_true();
  $state->{"calibration_mode"}=json_true();
  log_line("terminal CAL_END cleanup skipped: accepted write has an unconfirmed same-socket CAL_END");
 }
 write_state($state);
 log_line($err);
 if($PGAC_LOADED) {
