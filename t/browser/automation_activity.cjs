...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>{
   ...
   pgAutomationRenderProgress();
   check(slot('job')==='8–15 min'&&slot('batch')==='45 min–1.5 h'&&slot('job-label')==='this job left'&&slot('batch-label')==='whole batch left','readouts round to calm figures and label each range plainly');
