			...
			uint32_t prop_id /* in */, uint64_t value /* in */, drmModePropertyPtr prop /* in */, uint32_t flags)
{
	int success = 0;
	if (!prop || !prop_id) {
		ofLogError() << "DRM: Unable to request " << name << " missing property";
		return;
	}
...
//    uint32_t flags = DRM_MODE_ATOMIC_ALLOW_MODESET;	
	...
	if (success < 0) {
		...
		case DRM_MODE_PROP_ENUM:
			ofLog() << "DRM: atomic_request " << name << "(enum): {" << drm_enum_name(prop, value) << "} = " << value;
			break;
		     ...
		     drmModePropertyPtr *prop /* out */)
{
	...
	if (!proplist) {
		ofLogError() << "DRM: Unable to read properties for object " << object_id;
		return false;
	}
...
//		bool immutable = flags & DRM_MODE_PROP_IMMUTABLE;
		...
		if (strcmp(_prop->name, name) == 0) {
			...
			case DRM_MODE_PROP_ENUM:
			    ...
			    ofLog() << "DRM: " << name << "(enum): currently set to {"<< drm_enum_name(_prop, _value) << "} = " << _value;			
				break;
			case DRM_MODE_PROP_BITMASK:
				ofLog() << "DRM: " << name << ":";
				for (int j = 0; j < _prop->count_enums; ++j) {;
...
int foundHDR=0;
     ...
     * helper is mid-open at this renderer's open() instant, the
     * renderer becomes a plain client and drmSetMaster is EACCES
     * FOREVER for that fd - retrying the ioctl is futile. Close the
     * fd, reopen the device, then retry. Bounded: 20 tries x 250ms
     * after the initial 5s of plain retries. */
...
//	colorspace_on = true;
	...
	int panel_hdr_type = is_panel_hdr_dovi(device, connectorId);
	if ((dv_status || isDoVi || is_std_DoVi) && panel_hdr_type != HDR_TYPE_DOVI) {
		ofLog() << "DRM: active DoVi config selected; using DoVi-capable setup";
		panel_hdr_type = HDR_TYPE_DOVI;
	}

    if (panel_hdr_type == HDR_TYPE_HDR10) {
	...
	} else if (panel_hdr_type == HDR_TYPE_DOVI) {
...
{  //  ofShader shader;    
	...
	settings.shaderSources[GL_FRAGMENT_SHADER] = R"(
		...
		precision highp float;
		uniform vec4 globalColor;
		uniform vec3 source_rgb;
		uniform int source_max;
	//	uniform int bits;
	//	uniform int colorimetry;
	//	uniform int color_format;
	//	uniform int rgb_quant_range;
	...
	//	uniform int is_std_DoVi;
		...
		int sourceCode12(float rawCode)
		{
			int code = int(round(rawCode));
			if (source_max == 4095)
				return clamp(code, 0, 4095);
			if (source_max == 1023)
				return clamp(code, 0, 1023) << 2;
			return clamp(code, 0, 255) << 4;
		}

		void main() 
		{		
			int srcR = sourceCode12(source_rgb.r);
			int srcG = sourceCode12(source_rgb.g);
			int srcB = sourceCode12(source_rgb.b);
		//	float coeffs[5][3];
		//	coeffs[0] = float[](0.2126, 0.7152, 0.0722); //BT709
		//	coeffs[1] = float[](0.2627, 0.6780, 0.0593); //BT2020
		...
		//	coeffs[4] = float[](0.0, 0.0, 0.0); //dovi rgb2 coeff
			...
			int R1, G1, B1, R2, G2, B2;
				...
				/* YCrCb422 matrix */
				Y1 = round(coeffs_num.x * float(srcR) + coeffs_num.y * float(srcG) + coeffs_num.z * float(srcB));
				Cb = round((((-coeffs_num.x/coeffs_div.x) * float(srcR) - (coeffs_num.y/coeffs_div.x) * float(srcG) + coeffs_div.z * float(srcB)) * 2.0 * 224.0/219.0)/2.0 + 2048.0); // Chrominance Blue
				Y2 = Y1;
				Cr = round(((coeffs_div.z * float(srcR) - (coeffs_num.y/coeffs_div.y) * float(srcG) - (coeffs_num.z/coeffs_div.y) * float(srcB)) * 2.0 * 224.0/219.0)/2.0 + 2048.0); // Chrominance Red
				a = 1.0;
				/* Pack YUV for tunneling -- to do?? */
			

				/* Pack Dolby As RGB */
				int Y1Code = clamp(int(Y1), 0, 4095);
				int Y2Code = clamp(int(Y2), 0, 4095);
				int CbCode = clamp(int(Cb), 0, 4095);
				int CrCode = clamp(int(Cr), 0, 4095);
				R1 = CbCode >> 4;  
				G1 = Y1Code >> 4;
				B1 = (Y1Code & 15) | ((CbCode & 15) << 4);
				R2 = CrCode >> 4;  
				G2 = Y2Code >> 4;
				B2 = (Y2Code & 15) | ((CrCode & 15) << 4);	
  ...
  //          EGL_NONE};
    ...
    if(config)
    {
		...
		if (hdr_primaries == 2 || hdr_primaries == 0) {
			// P3-D65 primaries (per 1.6 device control template HDR preset).
			// The HDR metadata's primaries (set below via
			// DisplayChromacityList[2]) carry the P3-D65 chromaticity;
			// the EGL colorspace only controls the EOTF/quantization,
			// not the chromaticity. EGL has no native "P3 + PQ" colorspace,
			// so reuse BT2020_PQ -- this is the canonical 1.6 reference's
			// behavior (see ofxRPI4WindowHDR github master). Framebuffer
			// values must then be in BT.2020 RGB; the openFrameworks
			// pipeline produces them directly for pattern draws (greyscale
			// is identity through any chromaticity matrix for the autocal
			// 26pt greyscale series).
			if (static_cast<int>(eotf) == 2) {
				EGLint attribs[] = {EGL_GL_COLORSPACE_KHR,EGL_GL_COLORSPACE_BT2020_PQ_EXT,EGL_NONE };
				EGL_create_surface(attribs, config);
			} else {
		...
		switch (bit_depth) {
			...
			case 8:
				if (colorspace_on) {
				   ...
				   if ((dv_status || isDoVi) && !is_std_DoVi && starting_bpc >= 10) {
					   ofLog() << "DRM: preserving Low Latency DoVi output bpc of " << starting_bpc << " bits";
...
void ofxRPI4Window::swapBuffers()
{
 ...
 //  drmModeSetCrtc(device, crtc->crtc_id, fb->fb_id, 0, 0, &connectorId, 1, &mode);   
	 ...
	 * client.  The old code entered drmHandleEvent() unconditionally, so
	 * its blocking read then waited forever: commands continued to update
	 * operations.txt while the TV remained on the last (possibly white)
	 * framebuffer.
	 *
	 * Bound the wait.  Exiting is deliberate: the control plane already
	 * starts a missing renderer on the next pattern request, and closing
	 * the DRM fd is safer than continuing with an unknown scanout buffer.
	 */
...
void ofxRPI4Window::ResetConnectorProperties()
{
	...
	if (!ok) {
		colorimetry_property = "Colorspace";
		ok = drm_mode_get_property(device, connectorId, DRM_MODE_OBJECT_CONNECTOR, colorimetry_property, &prop_id, &colorimetry, &prop);
	}
	...
	// Full on every flip, which made RGB Limited PLUGE show superblacks
	...
	if (!ok) {
		rgb_quant_property = "Broadcast RGB";
		ok = drm_mode_get_property(device, connectorId, DRM_MODE_OBJECT_CONNECTOR, rgb_quant_property, &prop_id, &rgb_quant_range, &prop);
	}

	if (!ok || !(rgb_quant_range >=0)) { 
		ofLogError() << "DRM: Unable to find RGB Quant Range/Broadcast RGB";
	} else {
		uint64_t conf_q = (avi_info.rgb_quant_range == 1 || avi_info.rgb_quant_range == 2)
			? (uint64_t)avi_info.rgb_quant_range : 2;
		rgb_quant_range = (strcmp(rgb_quant_property, "Broadcast RGB") == 0)
			? pi5_broadcast_rgb_from_rgb_quant_range(conf_q)
			: conf_q;
		first_req = 1;
		last_req = 1;
...
void ofxRPI4Window::updateAVI_Infoframe(uint32_t plane_id, struct avi_infoframe avi_infoframe)
{
	...
	if (!ok || !(rgb_quant_range >=0)) { 
		...
		rgb_quant_range = (strcmp(rgb_quant_property, "Broadcast RGB") == 0)
			? pi5_broadcast_rgb_from_rgb_quant_range(avi_infoframe.rgb_quant_range)
			: avi_infoframe.rgb_quant_range;
		drm_mode_atomic_set_property(device, req, rgb_quant_property, connectorId, prop_id, rgb_quant_range, prop, 0);
	}	
