...
- **LG autocal (SDR and HDR)** — on-device closed-loop calibration of LG and compatible displays via `meter_lg_autocal.pl` (1D greyscale + RGB balance) and `meter_lg_3d_autocal.pl` (3D LUT), in both SDR and HDR10 modes. Drives the full greyscale → 1D LUT → color checker → 3D LUT → magic-wand polish pipeline from the Pi with no PC-side calibration software required.
